...
while(cap.isOpened()):
    ...
    if ret == True:
        ...
        if LCD_IMAGE_PIXEL_BIT == 8:
            ...
            # print(len(byte_arry), bytei)
            LCD_IMAGE_FILE.write(bytearray(np.array(byte_arry, dtype=np.uint8)))
            '''
